...
object MessageCollection {
    ...
    private fun addMessages(dto: CollectionDto) {
        ...
        dto.messages.run {
            add(MessageDto("m001", "I just got my licence everyone!", "u001", "f001", 1, POST))
            add(MessageDto("m002", "Selling my old car!", "u001", "f001", 2, POST))
            add(MessageDto("m003", "Anyone drinking tonight?", "u001", "f001", 3, POST))
            add(MessageDto("m004", "Lost my phone! - User sent from his iphone", "u001", "f001", 4, POST))
            add(MessageDto("m005", "Wanna hang out?", "u002", "f002", 5, MESSAGE))
            add(MessageDto("m006", "I know a place", "u002", "f002", 6, MESSAGE))
            add(MessageDto("m007", "Having a great time!", "u002", "f002", 7, MESSAGE))
            add(MessageDto("m008", "I love you...", "u003", "f003", 8, PRIVATE_MESSAGE))
            add(MessageDto("m009", "My heart aches", "u003", "f003", 9, PRIVATE_MESSAGE))
            add(MessageDto("m010", "PUBLIC ANNOUNCEMENT: EARTHQUAKE! HIDE!", "u004", "f004", 10, PUBLIC_MESSAGE))
        }

        assert(dto.messages.size == dto.messages.map { it.messageId }.toSet().size)
        assert(dto.messages.size == dto.messages.map { it.message}.toSet().size)
    }
}